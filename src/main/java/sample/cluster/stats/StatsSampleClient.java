...
public class StatsSampleClient extends AbstractActor {
	...
	public StatsSampleClient(String servicePath) {
		...
		tickTask = getContext().system().scheduler().schedule(interval, interval, self(), "tick",
...
//				.matchEquals("tick", t -> nodes != null, t -> {
					// just pick any one
				...
				}).match(StatsResult.class, System.out::println).match(JobFailed.class, System.out::println)
				.match(CurrentClusterState.class, state -> {
					...
					System.out.println("+++++++++++++++++++++++++++++++++++Member Up. If its a StatsService add "+mUp.member()+", "+mUp.member().getRoles());
					...
					if (reachable.member().hasRole("compute")){
						System.out.println("Reachable. add compute member: "+reachable.member());
